...
## Chiffrement du catalogue (usage personnel)

Le dépôt est public (contrainte de GitHub Pages en offre gratuite), mais les données
du catalogue sont **chiffrées** : pour un tiers, ce ne sont que des octets illisibles.

- Algorithme : **AES-256-GCM**, clé dérivée par **PBKDF2-SHA256** (250 000 itérations).
- Tout se passe dans le navigateur, via **WebCrypto** (natif — aucune dépendance).
- Le manifeste (`data/manifest.json`) reste en clair : il ne contient que les noms des
  modules et le sel de dérivation. Aucune donnée numismatique.
- L'app demande la phrase secrète **une seule fois par appareil**. La clé dérivée est
  ensuite conservée dans IndexedDB : les ouvertures suivantes sont transparentes,
  y compris hors-ligne.
- Les fichiers de données portent le suffixe `.enc`.

### Construire un catalogue chiffré

```bash
python3 outils/construire-catalogue.py
# → demande la phrase secrète (laisser vide = pas de chiffrement)
```

Ou sans saisie interactive :

```bash
MEDAILLIER_PHRASE="ma phrase secrète" python3 outils/construire-catalogue.py
```

**Attention** : chaque construction génère un nouveau sel. Si vous reconstruisez le
catalogue, tous les appareils redemanderont la phrase (la clé en cache devient
invalide). C'est normal.

**Si vous perdez la phrase**, les données sont irrécupérables — il faut reconstruire
`data/` depuis `sources/`. Gardez donc `sources/` (en clair, hors dépôt public ou dans
un dépôt privé).
...
| | |
|---|---|
| ✅ | Un visiteur du dépôt ne voit que des octets illisibles |
| ✅ | Aucun moteur de recherche n'indexe les données (`robots.txt` + `noindex`) |
| ✅ | Le catalogue n'est pas *republié* dans une forme exploitable |
| ⚠️ | Ce n'est pas un secret d'État : la phrase est saisie dans un navigateur |
| ⚠️ | Quiconque connaît la phrase peut tout lire |

C'est proportionné à l'objectif : un usage strictement personnel, accessible depuis
n'importe quel appareil avec un simple lien.
